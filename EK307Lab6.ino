...
int squareHalfCycSamples = 1; //number of analog samples betwen
...
void setup() {
  ...
  DDRD = DDRD | B11111100;    //make pins D2..D7 outputs
}
...
if(z == squareHalfCycSamples){  //when we reach the max then change state
 digitalWrite(squareWaveOut,outState);
  ...
  PORTD = portOut << 2; //bit shift the contents of portOut by 2
  ++ portOut; //increment the output. This make a simple oscillator 
  //because portOut overflows at 255. Each more significant bit has 1/2 the frequency of the bit before
